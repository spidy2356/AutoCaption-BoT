...
def edit_caption(bot, update: pyrogram.types.Message):
    if os.environ.get("custom_caption"):
        motech, mote = get_file_details(update)  # Assuming get_file_details returns mote as well
        try:
            update.edit(custom_caption.format(file_name=motech.file_name, mote=mote))
        except pyrogram.errors.FloodWait as flood_wait:
            asyncio.sleep(flood_wait.value)
            update.edit(custom_caption.format(file_name=motech.file_name, mote=mote))
